...
def format_segments(
    ...
    ``prefer_romanized`` uses a segment's ``romanized_text`` in place of its
    ``text`` when the model supplied one. The prompt asks for Latin script in
    ``text`` itself, but some responses put it in a separate field instead.
    ...
    for segment in segments:
        ...
        # When Latin script was asked for, a model may return it in its own
        # field rather than in `text`. Prefer it so --romanize is honoured
        # either way.
        if prefer_romanized and key == "text" and segment.romanized_text:
            text = segment.romanized_text
