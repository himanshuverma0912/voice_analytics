...
# ---------------------------------------------------------------------------
# Reading PromptHub's reply from a file instead of making the request
# ---------------------------------------------------------------------------
# A laptop that cannot reach the registry directly can often reach it some
# other way -- a proxy, a jump host, a browser. Feeding the saved reply in
# separates "can I reach PromptHub" from "can I read the entitlement", which
# are different problems with different owners.

def test_a_saved_response_is_accepted(tmp_path):
    import json as _json

    module = _entitlement()
    path = tmp_path / "usecase.json"
    ...
    body = module._load_saved_response(str(path))
    assert body["data"]["status"] == "APPROVED"


def test_a_missing_saved_response_says_so(tmp_path, capsys):
    module = _entitlement()
    assert module._load_saved_response(str(tmp_path / "nope.json")) is None
    assert "not found" in capsys.readouterr().err


def test_an_html_error_page_is_reported_as_such(tmp_path, capsys):
    """A curl through a proxy often returns HTML, not JSON. Saying "not
    readable JSON" without that hint sends the reader looking in the wrong
    place."""
    module = _entitlement()
    path = tmp_path / "usecase.json"
    path.write_text("<html><body>502 Bad Gateway</body></html>")

    assert module._load_saved_response(str(path)) is None
    assert "not readable JSON" in capsys.readouterr().err


def test_an_empty_file_is_reported_rather_than_crashing(tmp_path, capsys):
    module = _entitlement()
    path = tmp_path / "usecase.json"
    path.write_text("")
    assert module._load_saved_response(str(path)) is None
...
def test_saving_only_one_field_instead_of_the_whole_reply_is_caught(tmp_path, capsys):
    import json as _json

    module = _entitlement()
    path = tmp_path / "usecase.json"
    path.write_text(_json.dumps(["gemini-2.5-flash"]))
...
# ---------------------------------------------------------------------------
# Scoring and transcription use different models
# ---------------------------------------------------------------------------
# Gemini transcribes, Qwen scores, on different gateways. A shared default
# would quietly judge calls with the transcription model -- different scores,
# no warning, and nothing in the data to show why.

def test_the_two_model_defaults_are_different():
    from voice_analytics.config import Settings

    settings = Settings(LLM_BASE_URL="https://llm.internal/v1")
    assert settings.MODEL_NAME == "gemini-2.5-flash"
    assert settings.ANALYSIS_MODEL_NAME == "qwen3-30b-a3b-instruct"
    assert settings.MODEL_NAME != settings.ANALYSIS_MODEL_NAME


def test_each_can_be_overridden_independently(monkeypatch):
    from voice_analytics.config import Settings

    monkeypatch.setenv("ANALYSIS_MODEL_NAME", "some-other-scorer")
    settings = Settings(LLM_BASE_URL="https://llm.internal/v1")
    assert settings.ANALYSIS_MODEL_NAME == "some-other-scorer"
    assert settings.MODEL_NAME == "gemini-2.5-flash"      # untouched


def test_analyse_and_extract_default_to_the_analysis_model():
    """Unlike the originating service, which accepted a model_name argument
    and then hardcoded the model in the request anyway."""
    import re
    from pathlib import Path

    root = Path(__file__).resolve().parents[2] / "src" / "voice_analytics" / "cli"
    for command in ("analyse", "extract"):
        source = (root / f"{command}.py").read_text()
        assert "settings.ANALYSIS_MODEL_NAME" in source, command
        assert not re.search(r"args\.model or settings\.MODEL_NAME", source), command


def test_transcribe_still_defaults_to_the_transcription_model():
    from pathlib import Path

    source = (Path(__file__).resolve().parents[2] / "src" / "voice_analytics"
              / "cli" / "transcribe.py").read_text()
    assert "args.model or settings.MODEL_NAME" in source


def test_no_model_name_is_hardcoded_into_a_request():
    """The originating `call_llm` put the model name in the call itself, so the
    setting and the argument were both decorative."""
    import re
    from pathlib import Path

    src = Path(__file__).resolve().parents[2] / "src" / "voice_analytics"
    for path in src.rglob("*.py"):
        if path.name == "settings.py":
            continue                      # the defaults legitimately live there
        body = re.sub(r'(""".*?"""|#[^\n]*)', "", path.read_text(), flags=re.S)
        assert "qwen3-30b-a3b-instruct" not in body, path
        assert 'model="gemini' not in body, path
