...
# ---------------------------------------------------------------------------
# build-prompt --from-prompts-response
# ---------------------------------------------------------------------------
# A saved reply from the prompts endpoint, for a machine that cannot reach the
# registry. Unlike the committed snapshot this carries the real approved
# versions, so the approval rules have to be applied to it exactly as they
# would be to a live response.

_PROMPTS_REPLY = {
    "success": True,
    "data": {"prompts": [
        {"name": "base_prompt", "version": 3, "status": "APPROVED", "value": "RULES v3"},
        {"name": "base_prompt", "version": 4, "status": "PENDING", "value": "IN REVIEW"},
        {"name": "empathy", "version": 1, "status": "APPROVED", "value": "Empathy?"},
        {"name": "regulatory_risk", "version": 1, "status": "PENDING", "value": "IN REVIEW"},
    ]},
}


def _write_prompts(tmp_path, document):
    path = tmp_path / "prompts.json"
    path.write_text(json.dumps(document))
    return str(path)


def test_prompts_response_is_read_from_the_whole_body(tmp_path):
    from voice_analytics.cli.build_prompt import _load_prompts_response

    prompts = _load_prompts_response(_write_prompts(tmp_path, _PROMPTS_REPLY))
...
@pytest.mark.parametrize(
    "document",
    [
        _PROMPTS_REPLY,                                    # the whole reply
        _PROMPTS_REPLY["data"],                            # just `data`
        _PROMPTS_REPLY["data"]["prompts"],                 # a bare list
    ],
    ids=["whole-body", "data-only", "bare-list"],
)
def test_the_obvious_ways_of_saving_a_curl_all_work(tmp_path, document):
    from voice_analytics.cli.build_prompt import _load_prompts_response

    assert len(_load_prompts_response(_write_prompts(tmp_path, document))) == 4


def test_a_version_in_review_never_displaces_the_approved_one(tmp_path):
    """base_prompt v4 is PENDING; v3 is what must be used."""
    from voice_analytics.cli.build_prompt import _load_prompts_response
    from voice_analytics.prompthub import resolve_base_prompt

    prompts = _load_prompts_response(_write_prompts(tmp_path, _PROMPTS_REPLY))
    assert resolve_base_prompt(prompts) == "RULES v3"


def test_a_kpi_with_only_a_pending_version_is_skipped_not_used(tmp_path):
    from voice_analytics.cli.build_prompt import _load_prompts_response
    from voice_analytics.prompthub import resolve_kpi_prompts

    prompts = _load_prompts_response(_write_prompts(tmp_path, _PROMPTS_REPLY))
    resolved, skipped = resolve_kpi_prompts(prompts, ["empathy", "regulatory_risk"])

    assert [k.kpi_code for k in resolved] == ["empathy"]
    assert skipped == ["regulatory_risk"]


def test_an_empty_or_malformed_prompts_file_is_refused(tmp_path):
    from voice_analytics.cli.build_prompt import _load_prompts_response

    with pytest.raises(ValueError, match="no prompt list"):
        _load_prompts_response(_write_prompts(tmp_path, {"data": {}}))
    with pytest.raises(ValueError, match="no prompt list"):
        _load_prompts_response(_write_prompts(tmp_path, []))

    html = tmp_path / "err.json"
    html.write_text("<html>502 Bad Gateway</html>")
    with pytest.raises(ValueError, match="not readable JSON"):
        _load_prompts_response(str(html))

    with pytest.raises(ValueError, match="not found"):
        _load_prompts_response(str(tmp_path / "nope.json"))
