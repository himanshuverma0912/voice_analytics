...
_DAG = _ROOT / "examples" / "airflow" / "voice_analytics_pipeline.py"
...
def test_the_dag_does_not_import_the_compute_library():
    """The library reaches the DAG as a container image, never as an import.
    ...
    assert "voice_analytics" not in modules, (
        "The DAG imports the compute library. Anything it needs from it should "
        "be a CLI subcommand instead."
    )
